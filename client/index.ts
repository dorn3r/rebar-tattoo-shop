...
alt.onServer(Tattoshop_Events.TATTOOSHOP_OPEN, async() => {
    ...
    //Shop close
    webview.on('tattoshop:close', closeshop);
    //Shop preview/buy
    webview.on('tattoshop:preview', previewtattoo)
    webview.on('tattoshop:buy', buytattoo)
})
...
async function buytattoo(currenttattoo) {
    ...
    if (ped === mModel) {
        ...
        alt.emitServer(Tattoshop_Events.TATTOO_BUY, collection, overlay, price)
...
async function previewtattoo(currenttattoo) {
    ...
    if (ped === mModel) {
        ...
        alt.emitServer(Tattoshop_Events.TATTOO_PREVIEW, collection, overlay)
    } else {
        const collection = alt.hash(currenttattoo.Collection);
